...
import Modul15.Grocery.Products.Product;
...
public class ProductDatabase {
    ...
    public void removeProduct(UUID serial) {
        products.stream().filter((p) -> (p.getSerial().equals(serial))).forEachOrdered((p) -> {
            products.remove(p);
        });
    }
    ...
    public String getTotalPrice() {
        ...
        result = products.stream().map((p) -> p.getPrice()).reduce(result, (accumulator, _item) -> accumulator + _item);
